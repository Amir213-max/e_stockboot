...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ isDarkMode, toggleTheme }) => {
  ...
  const handlePdfUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'application/pdf') {
        setUploadError('عفواً، الملف المختار ليس PDF.');
        return;
    }
    ...
    try {
        ...
        // Save full text
        await db.saveDocs(fullText);
        setDocsLength(fullText.length);
  ...
  const handleClearDocs = async () => {
      if(window.confirm('⚠️ تحذير هام: سيتم حذف الدليل المرفق بالكامل.')) {
          const defaultLength = await db.resetDocs();
          setDocsLength(defaultLength);
